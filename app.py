...
try:
    ...
    if api_keys:
        ...
        def ask_gemini(prompt, max_retries=2):
            ...
            return None
        logger.info(f"Gemini AI enabled with {len(api_keys)} key(s)")
...
active_games, registered_players, user_names_cache, error_log = {}, set(), {}, []
...
class RateLimiter:
    def __init__(self, max_req, window_sec):
        self.max_req, self.window, self.requests = max_req, timedelta(seconds=window_sec), defaultdict(deque)
    def is_allowed(self, user_id):
        now = datetime.now()
        user_req = self.requests[user_id]
        while user_req and now - user_req[0] > self.window: user_req.popleft()
        if len(user_req) >= self.max_req: return False
        user_req.append(now)
        return True
...
def init_db():
    try:
        with get_db() as conn:
            ...
            conn.execute('CREATE INDEX IF NOT EXISTS idx_history_user ON game_history(user_id, played_at DESC)')
            conn.execute('CREATE INDEX IF NOT EXISTS idx_history_type ON game_history(game_type, played_at DESC)')
...
def get_user_profile_safe(user_id):
    ...
    try:
        ...
        if name:
            ...
            with get_db() as conn: conn.execute('UPDATE users SET display_name=? WHERE user_id=?', (name, user_id))
            return name
...
def handle_message(event):
    ...
    try:
        ...
        if not user_id or not text or not validate_input(text): return
        with players_lock:
        ...
        try:
            ...
            if result.get('correct') and question_num >= QUESTIONS_PER_GAME:
                ...
                if scores:
                    ...
                    with games_lock:
                        if game_id in active_games: del active_games[game_id]
                    line_bot_api.reply_message(event.reply_token, [TextSendMessage(text=f"صحيح! +{points} نقطة"), FlexSendMessage(alt_text="الفائز", contents=card, quick_reply=get_quick_reply())])
                else:
                    with games_lock:
                        if game_id in active_games: del active_games[game_id]
