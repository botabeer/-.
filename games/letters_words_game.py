...
class LettersWordsGame:
            ...
            letter_boxes.append({
                ...
                "contents": [
                    {"type": "text", "text": letter, "size": "xxl", "weight": "bold",
                     "color": self.C['cyan'], "align": "center"}
                ],
                ...
                "type":"box","layout":"vertical","contents":[
                    ...
                    self.create_3d_box([
                        {"type":"text",
                         ...
                         "size":"md","color":self.C['cyan_glow'],
                ...
                self.create_3d_box([
                    {"type":"box","layout":"horizontal","contents":[
                        {"type":"text","text":f"{rank} {data['name']}",
                         "size":"sm","color":self.C['cyan'] if i==1 else self.C['text'],
                         "weight":"bold" if i <= 3 else "regular","flex":3},
                        {"type":"text","text":str(data['score']),
                         "size":"lg" if i==1 else "md",
                         "weight":"bold",
                         "color":self.C['cyan'] if i==1 else self.C['text2'],
                         "align":"end","flex":1}
                    ]}
                ...
                "type":"box","layout":"vertical","contents":[
                    ...
                    self.create_3d_box([
                        ...
                        {"type":"text","text":winner[1]['name'],
                         "size":"xxl","weight":"bold","color":self.C['cyan'],
                         "align":"center","margin":"sm"},
                        {"type":"text","text":f"{winner[1]['score']} نقطة",
                         "size":"lg","weight":"bold","color":self.C['cyan_glow'],
                         "align":"center","margin":"md"}
                    ], self.C['card'], "24px", "xl"),
    ...
    def check_answer(self, answer, user_id, display_name):
        ...
        if not self.can_form_word(word, list(self.available_letters)):
            return {
                'response': TextSendMessage(text=f"⚠️ لا يمكن تكوين '{answer}' من الحروف"),
                'correct': False
            }
        ...
        # كلمة صحيحة فقط
        return {
            'response': TextSendMessage(text=f"✅ '{answer}' صحيحة!\n+{points} نقطة"),
            'correct': True
        }
